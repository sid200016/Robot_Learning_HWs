...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        if not torch.is_tensor(obs_unnormalized):
            obs_unnormalized = ptu.from_numpy(obs_unnormalized)
        if not torch.is_tensor(acs_unnormalized):
            acs_unnormalized = ptu.from_numpy(acs_unnormalized)
        if not torch.is_tensor(obs_mean):
            obs_mean= ptu.from_numpy(obs_mean)
        if not torch.is_tensor(obs_std):
            obs_std = ptu.from_numpy(obs_std)
        if not torch.is_tensor(acs_mean):
            acs_mean= ptu.from_numpy(acs_mean)
        if not torch.is_tensor(acs_std):
            acs_std = ptu.from_numpy(acs_std)
        if not torch.is_tensor(delta_mean):
            delta_mean= ptu.from_numpy(delta_mean)
        if not torch.is_tensor(delta_std):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        with torch.no_grad():
            prediction, _ = self.forward(obs, acs, obs_mean, obs_std, acs_mean, acs_std, delta_mean, delta_std) # TODO(Q1) get the predicted next-states (s_t+1) as a numpy array
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        loss = self.loss(self.delta_network(torch.cat([ptu.from_numpy(normalize(observations, obs_mean, obs_std)),
                ptu.from_numpy(normalize(actions, acs_mean, acs_std)),
                ], dim=1)),
                ptu.from_numpy(target),
